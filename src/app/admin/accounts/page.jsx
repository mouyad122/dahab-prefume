...
const PERMISSION_KEYS = [
  ...
  { key: 'can_manage_employees', label: 'إدارة الحسابات والصلاحيات' },
...
export default function AdminAccounts() {
  ...
  const getRoleLabel = (roleKey) => {
    switch (roleKey) {
      ...
      case 'manager': return 'مدير فرع / مشرف';
      case 'inventory': return 'مسؤول مخزون';
      case 'accountant': return 'محاسب';
      default: return 'موظف كاشير';
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-sm">
            إدارة حسابات وصلاحيات النظام (المدراء، الكاشير، المحاسبين، ومسؤولي المخزون)
          </p>
        </div>
        <LuxuryButton 
          variant="primary"
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-20 text-[var(--color-text-muted)]">
              <p>لا يوجد حسابات مضافة حاليًا</p>
            </div>
          ) : (
            <table className="w-full text-right text-sm">
              <thead className="bg-[var(--color-bg-surface)] text-[var(--color-text-muted)] text-xs sticky top-0 z-10 shadow-sm border-b border-[var(--color-border)]">
                <tr>
                  <th className="py-3 px-5 font-normal">الاسم المعروض</th>
                  <th className="py-3 px-5 font-normal">اسم المستخدم</th>
                  <th className="py-3 px-5 font-normal">الدور الوظيفي</th>
                  ...
                  <tr key={emp.id} className="hover:bg-[var(--color-bg-surface)] transition-colors group">
                    ...
                    <td className="py-3 px-5">
                      <span className={`px-2 py-0.5 rounded text-[0.65rem] font-bold ${
                        emp.role === 'manager' || emp.role === 'admin' 
                          ...
                          : 'bg-[var(--color-gold-dim)] text-[var(--color-gold-light)]'
                      }`}>
                    ...
                    <td className="py-3 px-5">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {emp.permissions?.can_view_inventory && <span className="px-2 py-0.5 rounded bg-[var(--color-bg-primary)] border border-[var(--color-border-subtle)] text-[0.65rem]" title="المخزون">🏭 مخزون</span>}
                      </div>
                    </td>
                    <td className="py-3 px-5">
                      {emp.is_active ? (
          ...
          <form onSubmit={handleSubmit} className="glass-card w-full max-w-lg p-6 border border-[var(--color-border-strong)] flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-[var(--color-border)] pb-3">
              <h2 className="font-display text-xl font-bold text-[var(--color-gold-light)]">
                {editEmployeeId ? 'تعديل بيانات الحساب' : 'إضافة حساب جديد'}
              </h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="form-label">الاسم المعروض (الاسم الكامل)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="form-label">الدور الوظيفي للحساب</label>
                <select 
              ...
              <div>
                <label className="form-label border-b border-[var(--color-border)] pb-2 mb-3 block">صلاحيات الحساب</label>
